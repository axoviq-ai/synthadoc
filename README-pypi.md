...
| **[Sensitive Data Retract](https://github.com/axoviq-ai/synthadoc/blob/main/docs/user-quick-start-guide.md#sensitive-data-retract)** — scan all wiki pages for API keys, emails, SSNs, credit cards, and custom patterns; apply `[REDACTED]` in place; background scan runs automatically on a weekly schedule (async, pauses while the job queue drains); full audit trail records slug, pattern names, and counts — never the sensitive values themselves | **Yes**   | No                       | No                     | No                                                                                                  |

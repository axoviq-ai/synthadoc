...
| **[Agentic maintenance workflows](https://github.com/axoviq-ai/synthadoc/blob/main/docs/user-quick-start-guide.md#agentic-workflows)** — five conversational workflows (re-ingest stale pages, re-ingest by slug, broken wikilinks scan and fix, lint report, scaffold regeneration) via web UI, Obsidian query modal, or `synthadoc query` CLI; confirm-before-act gate on all destructive operations; graph sidebar chips trigger workflows with one click — no terminal required | **Yes** | No | No | No |

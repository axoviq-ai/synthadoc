...
<a href="https://github.com/axoviq-ai/synthadoc"><img src="https://img.shields.io/badge/Community%20Edition-v1.3.5-brightgreen.svg" alt="Version"/></a>
</p>
...
| **[Cross-wiki federation](https://github.com/axoviq-ai/synthadoc/blob/main/docs/design.md#40-cross-wiki-queries-v140)** — `synthadoc query --cross-wiki` fans out a single query across all registered running wikis; LLM auto-routes to relevant wikis; graceful degradation when wikis are offline | **Yes**            | No          | No         | No        |
...
| **[Cross-wiki federation](https://github.com/axoviq-ai/synthadoc/blob/main/docs/design.md#40-cross-wiki-queries-v140)** — fan-out across registered wikis; LLM routing; `[[wiki-name::PageTitle]]` citation pills; amber offline banner; operation-detection footnote | **Yes**   | No          | No         | No        |
...
| **[Citation faithfulness audit](https://github.com/axoviq-ai/synthadoc/blob/main/docs/user-quick-start-guide.md#checking-citation-faithfulness)** — on-demand LLM verification that each claim's text is actually supported by its cited source lines; per-page result cache with staleness detection; findings surfaced in the Obsidian source viewer                                                                                                                                                                                                                                                                                          | **Yes**   | No                       | No                     | No                                                                                                  |
...
| **[Agentic maintenance workflows](https://github.com/axoviq-ai/synthadoc/blob/main/docs/user-quick-start-guide.md#agentic-workflows)** — seven conversational workflows (stale-page re-ingest, broken wikilinks fix, lint report, scaffold regeneration, **contradiction resolver**, **orphan resolver**, **[broken citation resolver](https://github.com/axoviq-ai/synthadoc/blob/main/docs/user-quick-start-guide.md#demo--broken-citation-resolver)**) via web UI or `synthadoc workflow` CLI; confirm-before-act gate on all writes; graph sidebar chips trigger workflows with one click                                                                                                    | **Yes**   | No                       | No                     | No                                                                                                  |
| **[Sensitive Data Retract](https://github.com/axoviq-ai/synthadoc/blob/main/docs/user-quick-start-guide.md#step-27--scan-and-retract-sensitive-data)** — scan all wiki pages for API keys, emails, SSNs, credit cards, and custom patterns; apply `[REDACTED]` in place; background scan is incremental (only pages modified since the last cycle are checked); server auto-scans ingested pages immediately after each ingest job completes; `--changed-only` CLI flag for incremental manual runs; `synthadoc retract status` captures the full retraction audit trail (slug, pattern names, counts — never the sensitive values themselves) | **Yes**   | No                       | No                     | No                                                                                                  |
...
![Synthadoc Architecture](https://raw.githubusercontent.com/axoviq-ai/synthadoc/main/docs/png/architecture-overview.png)
...
→ Provider list and API key setup: [Quick-Start Guide — Appendix C](https://github.com/axoviq-ai/synthadoc/blob/main/docs/user-quick-start-guide.md#appendix-c--switching-llm-providers)
...
→ Routing performance benchmarks: [Quick-Start Guide — Appendix H](https://github.com/axoviq-ai/synthadoc/blob/main/docs/user-quick-start-guide.md#appendix-h--bm25-routing-performance-benchmarks)
→ Context packs and MCP integration: [docs/design.md — Context packs](https://github.com/axoviq-ai/synthadoc/blob/main/docs/design.md#context-packs)
→ Knowledge gap workflow: [docs/design.md — Knowledge gap workflow](https://github.com/axoviq-ai/synthadoc/blob/main/docs/design.md#knowledge-gap-workflow)
→ Semantic re-ranking (optional vector search): [docs/design.md — Semantic re-ranking](https://github.com/axoviq-ai/synthadoc/blob/main/docs/design.md#semantic-re-ranking)
...
For full architecture detail see [Cross-Wiki Queries in the design doc](https://github.com/axoviq-ai/synthadoc/blob/main/docs/design.md#40-cross-wiki-queries-v140).
...
# Orphan resolver — find pages with no incoming wikilinks and insert natural links
synthadoc workflow run --name orphan-resolver -w my-wiki

# Resolve only one orphaned page by slug
synthadoc workflow run --name orphan-resolver --slug ada-lovelace -w my-wiki

# Broken citation resolver — fix broken ^[file:L-L] source markers (broken_ref / malformed / out_of_range)
synthadoc workflow run --name broken-citation-resolver -w my-wiki

# Fix citations on a single page only
synthadoc workflow run --name broken-citation-resolver --slug my-page -w my-wiki
```
...
→ Full walkthrough: [Quick-Start Guide — Step 27](https://github.com/axoviq-ai/synthadoc/blob/main/docs/user-quick-start-guide.md#step-27--scan-and-retract-sensitive-data)

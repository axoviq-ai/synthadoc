...
27. [Scan and retract sensitive data](#step-27--scan-and-retract-sensitive-data)
...
# default = { provider = "groq",      model = "openai/gpt-oss-20b" }               # Groq (paid, $0.075/$0.30 per 1M)
...
lint    = { provider = "groq",   model = "openai/gpt-oss-20b" }  # cheaper for lint
...
The file lives at `~/.synthadoc/CROSS_WIKI_ROUTING.md`. After `routing init`, it looks like this:
...
The `default` section (empty `keywords:`) is the fallback — it receives any query that doesn't match a keyword rule. Edit the file to add named sections that pin topic areas to specific wikis:

```markdown
# Cross-Wiki Routing

## finance
wikis: finance-wiki, banking-wiki
...
## ops
wikis: ops-wiki
keywords: deployment, runbook, incident, SLA, outage
...
The first section whose `keywords:` list contains a word found in the query is used; if none match, the `default` section applies. When the file is absent or cannot be parsed, the system falls back to LLM auto-routing and logs a warning.

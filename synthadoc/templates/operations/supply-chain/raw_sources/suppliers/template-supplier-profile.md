---
# Copyright (c) 2026 William Johnason / axoviq.com. All rights reserved.
---
# Supplier Profile: [Supplier Name]

> **How to use this form**
>
> This template follows the **ISO 9001:2015 Clause 8.4 (Control of Externally Provided Processes)** -- the international standard requirement for evaluating, selecting, and monitoring external suppliers.
> If you already have a supplier qualification record in another format (AVL spreadsheet, ERP export, audit report), skip this form and ingest your document directly.
>
> Reference: [ISO 9001:2015](https://www.iso.org/iso-9001-quality-management.html)
>
> 1. Copy this file and rename it (e.g. `SUP-042-acme-steel.md`)
> 2. Fill in all sections
> 3. Run: `synthadoc ingest raw_sources/suppliers/<supplier-code>-<name>.md -w <wiki>`

---

## Supplier Identity

- **Supplier code**: `SUP-NNN`
- **Company name**: 
- **Category**: _(Raw material / Component / Sub-assembly / MRO / Logistics / Services / Other)_
- **Supply chain tier**: _(Tier 1 / Tier 2 / Tier 3)_
- **Approved status**: _(Approved / Conditional / Probationary / Disqualified)_
- **AVL approval date**: 

## Contact Information

- **Primary contact name**: 
- **Title**: 
- **Email**: 
- **Phone**: 
- **Address**: 

## Supply Profile

- **Critical parts or materials supplied** _(list top 3–5)_:
  1. 
  2. 
  3. 
- **Sole source**: Yes / No — _(If Yes, document contingency plan below)_
- **Lead time** (standard): 
- **Lead time** (expedited): 
- **Minimum order quantity**: 

## Commercial Terms

- **Payment terms**: 
- **Contract expiry**: 
- **Contract reference**: 
- **Annual spend** (last 12 months): $

## Quality

- **Quality certification**: _(ISO 9001 / IATF 16949 / AS9100 / None)_
- **Certificate expiry**: 
- **Last supplier audit date**: 
- **Audit result**: _(Pass / Pass with conditions / Fail)_
- **Current DPPM** (defects per million): 
- **Current on-time delivery rate**: %
- **Open corrective actions**: 

## Risk

- **Supply disruption risk**: _(Low / Medium / High)_
- **Geographic / geopolitical risk**: 
- **Single-site dependency**: Yes / No
- **Contingency supplier**: 

## Notes

_(Relationship history, key account manager, pending changes, strategic importance.)_

...
# Three precisely scoped false claims give the adversarial gate enough material
# to fire (adversarial_max_per_page = 3 → exactly 3 warnings ≥ threshold = 3),
# while remaining correctable in a single content-rewrite pass.  After the
# resolver fixes all three the scoped re-lint returns 0 warnings so the
# resolver can transition the page back to active without a second attempt.
_TEST_PAGE_CONTENT = """\
---
title: Live Test Page — Contradiction Resolver
status: draft
---
...
The sections below contain three deliberate factual errors designed to
trigger the adversarial gate.  All other content is accurate.  The
contradiction resolver should correct all three errors in a single
content-rewrite pass; the scoped re-lint that follows should then find
zero warnings so the resolver can promote the page back to active.

## Accurate background

The ENIAC (1945) was one of the first programmable general-purpose electronic
computers.  The transistor, invented at Bell Labs in 1947, eventually replaced
vacuum tubes and made modern electronics possible.

## Deliberate errors

**Error 1 — Space exploration:** The Apollo 11 mission in 1969 was the first
crewed spacecraft to land on Mars.  Neil Armstrong became the first human to
set foot on another planet.

**Error 2 — Physics:** The speed of light in a vacuum is 3,000 kilometres per
second, as confirmed by the Michelson–Morley experiment.

**Error 3 — Computing history:** The World Wide Web was invented by Vint Cerf
in 1975 as part of the ARPANET project.
"""
    ...
    try:
        ...
        if state != "contradicted":
            pytest.skip(
                ...
                "The test page contains 3 precisely scoped false claims — "
                "the LLM should flag all three reliably when adversarial_max_per_page = 3. "

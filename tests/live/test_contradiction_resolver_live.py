...
def test_case1_gate_demoted_fix():
    ...
    try:
        ...
        assert "Fixed" in output or "active" in output.lower(), (
            f"Expected fix confirmation in resolver output:\n{output[:2000]}"
        )
...
def test_case2_source_conflict_fix():
    ...
    try:
        ...
        output = result.stdout + result.stderr
        assert "Fixed" in output or "active" in output.lower(), (
            f"Case 2: expected fix confirmation for {target!r}:\n{output[:2000]}"
        )
        assert _page_state(target) == "active", (
            f"Case 2: {target!r} should be active after resolver"
        )
    finally:
...
def test_case3_multi_page_all_scope():
    ...
    try:
        # Input: approve scope (a=all), cost estimate, then diff + continue for each page
        result = _run_workflow(
            input_text="a\ny\ny\ny\ny\ny\ny\n",
            timeout=480,
        )
...
def test_case5_cli_parity():
    ...
    try:
        ...
        assert "Fixed" in output or "active" in output.lower() or result.returncode == 0, (

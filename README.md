...
| **[Sensitive Data Retract](docs/user-quick-start-guide.md#sensitive-data-retract)** — scan all wiki pages for API keys, emails, SSNs, credit cards, and custom patterns; apply `[REDACTED]` in place; background scan is incremental (only pages modified since the last cycle are checked); server auto-scans ingested pages immediately after each ingest job completes; `--changed-only` CLI flag for incremental manual runs; `synthadoc retract status` captures the full retraction audit trail (slug, pattern names, counts — never the sensitive values themselves) | **Yes**   | No                       | No                     | No                                                                                                  |

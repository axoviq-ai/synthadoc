...
26. Agentic maintenance workflows — six conversational workflows via web UI, Obsidian query modal, or CLI: re-ingest stale pages (bulk or by slug), scan and fix broken wikilinks, run lint and view the full report, run scaffold and regenerate wiki files, resolve contradicted pages interactively (diff shown before every write, human approval required), resolve orphaned pages by inserting natural wikilinks into related content — agent confirms before touching anything, streams inline progress
